document.addEventListener("DOMContentLoaded", function() {
  ...
  function show_results(data){
    ...
    intensityEl.innerHTML = data.required_irrigation_intensity ? `не менее ${data.required_irrigation_intensity} ` : ""
    Q_el.innerHTML = data.Q ? `не менее ${data.Q}` : ""
    duration_el.innerHTML = data.duration_min ? `не менее ${data.duration_min}` : ""
    distance_el.innerHTML = data.distance_max ? `${data.distance_max}` : ""
    S_el.innerHTML = data.S ? `не менее ${data.S}` : ""
    ...
    sprinklers_select.addEventListener("change", function() {
      const
        ...
        sprinkler_K_value = K
        H.innerHTML = `${pwork_meter.toFixed(2)}`        // убираю размерность из формы для html 
        Q1.innerHTML = `${Q.toFixed(2)}`
        sprinkler_K.innerHTML = `${K}`
      ...
      <div id="connection-results-${connectionNum}" style="display: none;">
        <table class="table">
          <thead>
            ...
            <th class="text-center">Потери давления, м вод. ст.</th>
            <th class="text-center">Давление на выходе, м вод. ст.</th>
          </thead>
          <tbody>
            <tr>
    ...
    return `
      <table class="table branchTable" id="branchTable-${branchNum}">
        <thead>
          ...
          <th style="width: 100px" class="text-center">Диаметр</th>
          <th style="width: 100px" class="text-center">Длина, м</th>
          <th style="width: 100px" class="text-center">Высотная отметка, м</th>
          <th style="width: 100px" class="text-center">Скорость потока, м/с</th>
          <th class="text-center">Потери давления, м вод. ст.</th>
  ...
  function initConnection(connectionNum) {
    ...
    function calculateConnection(connectionNum) {
      ...
      //новое 
      const nextRow = document.getElementById(`node-row-${connectionNum+1}`)
      ...
      // alert(Q_in)
      // alert(P_in)
      let Q_prev_adjusted  = 0
      let Q_next_adjusted = 0
      ...
      if (nextBranchResults) {
        ...
        Q_prev_adjusted = Q_in
        Q_next_adjusted = Q_next
      ...
      if (length === 0 || !connection_kt_values[connectionNum]) {
        ...
        if (nodeRow) {
          const nodeLength = nextRow.querySelector('.node-length')
          const nodePLoss = nextRow.querySelector('.node-P-loss')
          const nodeQ = nextRow.querySelector('.node-Q')
          const nodeP = nextRow.querySelector('.node-P')
      ...
      const P_loss = (Q_prev_adjusted * Q_prev_adjusted * length) / connection_kt_values[connectionNum]
      const P_out = P_in + P_loss

      // Отображаем результаты в блоке соединения
      document.getElementById(`connection-Q-in-${connectionNum}`).innerHTML = `${Q_prev_adjusted.toFixed(2)}`
      ...
      // Отображаем результаты в строке узла таблицы предыдущей ветви //следующей
      if (nextRow) {
        // const nodeLength = nextRow.querySelector('.node-length')
        // const nodePLoss = nextRow.querySelector('.node-P-loss')
        const nodeQ = nextRow.querySelector('.node-Q')
        ...
        if (nodeQ) nodeQ.innerHTML = `${Q_prev_adjusted.toFixed(2)} + ${Q_next_adjusted.toFixed(2)} = ${Q_in.toFixed(2)}`
        if (nodeP) nodeP.innerHTML = `${P_out.toFixed(2)}`
      }

      // Сохраняем результаты для следующей ветви
      branch_results[connectionNum+1] = {
        Q_total: Q_in,
        P_total: P_out
      }
...
function get_tubes(i) {
      const TubeType = document.getElementById(`TubeType-${i}`);
      console.log("1404",TubeType)
      const TubeTypeValue = TubeType.value;
      if(TubeTypeValue){
        fetch(`/api/tubes/list/?type_id=${TubeTypeValue}`)
          .then(response => {
            if (!response.ok) {
          ...
          .then(data => {
            data_global[`data_global${i}`]=data
            console.log(data)
          })
          .catch(error => {
            console.error('Fetch error:', error);
...
const form_main = document.getElementById('mainForm');

form_main.addEventListener('submit', function (e) {
    e.preventDefault();

    fetch(form_main.action, {
        method: form_main.method,
        body: new FormData(form_main),
        headers: {
...
function button_create1(id_branch) {
    return `
        <div class="row">
          <div class="col-md-6">
            <div class="row my-4">
              <label for="group" class="col-sm-4 col-form-label">Тип трубы</label>
              <div class="col-sm-8">
                <select class="form-select" required name="TubeType" id="TubeType-${id_branch}">
                  <option value=""></option>
                </select>
              </div>
            </div>
          ...
          <div class="col-md-6">
            <div class="row my-4">
              <label for="group" class="col-sm-10 col-form-label">Количество оросителей на ветви</label>
              <div class="col-sm-2">
            <input type="number"
                   class="form-control auto-update"
                   id="count_sprink${id_branch}"
                   name="temp"
      ...
      <div id="inner${id_branch}"></div>
    `;
}
async function create_inner(i) {
    const before = 0;
...
async function initializeInitialBranches() {
    ...
    const cout_branch = document.getElementById('count_branch').value
    var last_branch = 0;
    for (let i = 1; i <= cout_branch; i++) {
      // Создаем таб ветви
      const branchTab = document.createElement('li')
      branchTab.className = 'nav-item'
      branchTab.id = `branch_tab${i}`
      ...
      // tabList.add(branchTab)
      tabList.appendChild(branchTab);
      ...
      branchPane.innerHTML = button_create1(i);
      ...
      initBranch(i);
      console.log(`i=${i}`);
      const buttonBranch = document.getElementById(`count_sprink${i}`);
      const TubeType = document.getElementById(`TubeType-${i}`);
      console.log(TubeType);
      // Правильно: оборачиваем в стрелочную функцию
      buttonBranch.addEventListener('change', () => create_inner(i));
      TubeType.addEventListener('change', () => get_tubes(i));
    }
  }
// initializeInitialBranches
  ...
  async function loadTubeTypesFromAPI(branchNum) {
    ...
    const tubeTypeSelect1 = document.getElementById(`TubeType-а`)
    console.log(`Не найденно TubeType-${branchNum}`)
    console.log(`Не найденно ${tubeTypeSelect}`)
    console.log(`Не найденно ${tubeTypeSelect1}`)
...
const button_id1 = document.getElementById('button_id_ayp')
  // Функция создания начальных ветвей
button_id1.addEventListener('click', initializeInitialBranches)
