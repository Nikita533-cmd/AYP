...
class CalculationForm(forms.Form):                       
    Q= forms.FloatField(label=mark_safe('Расход '' <img src="/static/app/img/vop_ico13.png" ''title="Введите расход в м³/час" ''style="cursor: help; width:18px; height:18px; vertical-align:middle;">'),widget=forms.NumberInput(attrs={'id': 'Q', 'class':'wiget'}), min_value=0, max_value=3000)
    ...
    H = forms.FloatField(label=mark_safe('Напор '' <img src="/static/app/img/vop_ico13.png" ''title="Введите напор в метрах водяного столба" ''style="cursor: help; width:18px; height:18px; vertical-align:middle;">'), widget=forms.NumberInput(attrs={'id': 'H', 'class':'wiget'}) ,min_value=0, max_value=160)
    istochik =forms.ChoiceField(
        label=mark_safe('Источник водоснабжения '' <img src="/static/app/img/vop_ico13.png" ''title="Выберите источник водоснабжения. Внимание! Для варианта «Резервуар»: если уровень воды в резервуаре выше минимального — используйте знак &quot;-&quot;." ''style="cursor: help; width:18px; height:18px; vertical-align:middle;">'),
    ...
    DN_in = forms.ChoiceField(
        label=mark_safe('Входной диаметр '' <img src="/static/app/img/vop_ico13.png" ''title="Выберите диаметр на входе в МПНУ, список огранчиен в соответствии с СП485. п.6.7.1.37" ''style="cursor: help; width:18px; height:18px; vertical-align:middle;">'),
    ...
    DN_out = forms.ChoiceField(
        label=mark_safe('Выходной диаметр '' <img src="/static/app/img/vop_ico13.png" ''title="Выберите диаметр на выходе из МПНУ, список огранчиен в соответствии с СП485. п.6.7.1.37" ''style="cursor: help; width:18px; height:18px; vertical-align:middle;">'),
        choices=[],  # сюда нужно добавить варианты выбора (value, label)
    ...
    label=mark_safe('Жокей насос '' <img src="/static/app/img/vop_ico13.png" ''title="Жокей-насос необходим для поддержания давления в системе пожаротушения и обеспечения срабатывания узла управления, который контролирует запуск основных пожарных насосов." ''style="cursor: help; width:18px; height:18px; vertical-align:middle;">'),
    required=False,
